...
async function uploadPDFRouteNew(req, res) {
  try {
    const { category_id, blogCount, mimetype, authorName: author, bookTitle: book_name, imageUrl: book_image, } = req.body;
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to upload PDF to storage" });
    }
    ...
    const bookEntryData = { user_id: verifiedToken.sub, author, book_image, book_name, pdf_link, category: category_id, };
